...
void initBoss(int argc, char* argv[]) {
  ...
  for (int ii=0; ii <= sizeof(progs)/sizeof(char*); ii++)
    sem_init(&(mymap->progs[ii].sync), SSHARE, SSYNC);
...
// Kontributor: rekapaska, shanniadm, tolhassianipar
// Harap UPDATE tanggal revisi!
// Thu Jul  2 14:54:54 WIB 2020
// Fri Jul  3 17:27:14 WIB 2020
// Tue Jul  7 12:52:35 WIB 2020
void doBoss(int entry) {
  for (int ii=0; ii <= sizeof(progs)/sizeof(char*); ii++)
    sem_post(&(mymap->progs[ii].sync));
}

// Tambahkan di sini nama para kontributor (akunGitHub)
// Harap UPDATE tanggal revisi!
// Thu Jul  2 14:54:54 WIB 2020
// Tue Jul  7 10:10:27 WIB 2020
